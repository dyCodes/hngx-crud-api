...
exports.getAll = async (req, res, next) => {
	...
	} else return next(res.json({ message: "Error fetching persons" }));
};
...
exports.getPerson = async (req, res, next) => {
	...
	if (!person) {
		return next(res.status(404).json({ message: "Person not found" }));
	}
	// Return person
...
exports.addPerson = async (req, res, next) => {
	...
	if (personExists) return next(res.status(400).json({ message: "Person already exists" }));
	...
	} else return next(res.status(400).json({ message: "Error creating person" }));
};
...
exports.updatePerson = async (req, res, next) => {
	...
	if (!person) return next(res.status(404).json({ message: "Person not found" }));

	// Update person
	person.name = name;
	const updatedPerson = await person.save().catch((err) => console.error(err));
	if (updatedPerson) {
		return next(res.status(200).json(updatedPerson));
	} else return next(res.status(400).json({ message: "Error updating person" }));
};
